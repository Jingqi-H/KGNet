...
# 根据【gloria-main】https://github.com/marshuang80/gloria，知道global_loss是怎么计算的
# global loss 和local loss计算的是对比损失，我想看的是相似度计算的地方，也就是local loss的部分，
# 如何获得attention weight
"""
...
def attention_fn(query, context, temp1):
    ...
    """
    print(query.shape, context.shape, temp1)
...
def global_loss(cnn_code, rnn_code, eps=1e-8, temp3=10.0):
    ...
    # 忽略batch维度，进行矩阵相乘
    print('cnn_code:{}, rnn_code:{}'.format(cnn_code.shape, rnn_code.shape))
...
if __name__ == '__main__':
    # img_em, text_em = torch.randn([3, 5]), torch.randn([3, 5])
    # print(img_em, '\n', text_em)

    word = torch.randn([48, 768, 25])
    context = torch.randn([48, 768, 19, 19])
    weiContext, attn = attention_fn(
        word, context, 4.0
    )  # [48, 768, 25], [48, 25, 19, 19]
    print('weiContext:{}\nattn:{}'.format(weiContext.shape, attn.shape))