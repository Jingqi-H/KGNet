...
from utils.visulize import plot_loss_acc, plot_lr
...
def setup_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
    # cudnn中对卷积操作进行了优化，牺牲了精度来换取计算效率。如果需要保证可重复性，可以使用如下设置:True
...
def main(parser):
    ...
    for i in range(args.k_fold):
        ...
        attri_criterion = nn.CrossEntropyLoss().cuda()
        ...
        train_part_loss = {'total_loss': [], 'cla_loss': [], 'seg_loss': [], 'em_loss': [], 'attri_loss': []}
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            train_dict = train(args,
                               ...
                               attri_criterion=attri_criterion)
            ...
            train_part_loss['attri_loss'].append(train_dict['loss_attri'])
        ...
        save_k_final_results(save_display_dir,
                             i, val_dict['val_gt_labels'], val_dict['val_pred_labels'],
                             val_dict['val_pred_probs'], cla)
        if not args.continue_my_model:
            plot_lr(lr_epoch)
            plt.savefig(os.path.join(save_display_dir, 'lr_' + 'K' + str(i + 1) + '.png'))
