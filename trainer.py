...
def train(args,
    ...
    per_loss, per_seg_loss, per_cla_loss, per_attri_loss = [], [], [], []
    tt_seg, tt_disc = [], []
    ...
    for step, data in enumerate(data_loader, start=0):
        ...
        # print(cla_label)
        attri_loss = attri_criterion(attri_sim, cla_label)
        # print('attri_loss:', attri_loss)

        loss = args.p_seg * seg_loss + args.p_disc * disc_loss[
            'loss'] + args.p_cla * cla_loss + args.p_attri * attri_loss
        loss.backward()
        ...
        per_attri_loss.append(args.p_attri * attri_loss.item())
        per_cla_loss.append(args.p_cla * cla_loss.item())
    ...
    print('total Loss: {:.4f}'.format(np.mean(per_loss)))
    print(
        'seg loss: {:.4f}/{:.4f} | discriminative loss: {:.4f}/{:.4f} | cla loss: {:.4f} | attri loss:{:.4f}'.format(
            np.mean(per_seg_loss),
            np.mean(tt_seg),
            np.mean(per_discriminative_loss),
            np.mean(tt_disc),
            np.mean(per_cla_loss),
            np.mean(per_attri_loss),
        ))
    ...
    t_result['loss_attri'] = np.mean(per_attri_loss)
    return t_result
